...
_CIFAR_FS_DATASET_DIR = 'datasets/CIFAR_FS'
...
class CIFAR_FS(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        if self.phase=='train':
            ...
            if self.phase=='test':
                # load data1 that will be used for evaluating the recognition
                # accuracy of the base categories.
                data_base = load_data(file_train_categories_test_phase)
                # load data1 that will be use for evaluating the few-shot recogniton
            ...
            self.data = np.concatenate(
                [data_base['data1'], data_novel['data1']], axis=0)
...
class FewShotDataloader():
    ...
    def createExamplesTensorData(self, examples):
        """
        Creates the examples image and label tensor data1.
